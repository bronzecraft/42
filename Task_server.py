...
# search singal file 
 ...
 # [file path, file,file time]     
        return file_list
# move job files to the dirs        
#     def task_move(self):
# 		# path = '//s4nas02.ap.cat.com/ITDD_Groupdata02/desk_hpc'
#         #source path
#         path = "\\s4nas02.ap.cat.com\\ITDD_Groupdata02\\desk_hpc\\ServerA"
#         while self.n > 0:
# 		# find .lck file to check jobs
#             tasklist_singal = self.singal_search('.lck', path)
#         # tasklist_txt = list(map(lambda y : y.replace(".singal",".txt"), tasklist_singal))
#         # target path
#             trp = "E:/WeiLi/Personal/Python/Server/Test"
#             n_list = len(tasklist_singal)
#             for i in range(n_list):            
#                 file = tasklist_singal[i][1].replace(".lck",".json")
# 					#job's .json file path
...
#                 os.remove(file_path_singal)
#                 with self._value_lock:
#                     ts = os.path.join(tgd,file)
#                     self.task_list_waiting.append([file,"waiting",ts])
#             time.sleep(5)
#             print ("waiting")
#generate run bat file according to solver type
    def task_solver(self,data):
        with open ("run.bat","w") as f:
            if data["solver"] == "abaqus" :
                print ("echo ask_delete=OFF >abaqus_v6.env",file = f )
                print ("call call C:/SIMULIA/Commands/abaqus.bat job=" + str(data["job_name"])+" "+str(data["args"])+" " +"cpus=56 " +"int",file = f )
