 ...
 # [file path, file,file time]     
    ...
    # if os.path.exists(task['account'])
                ...
                with self._value_lock:
                    #[ job_id, job_name,user_name,charge_number,solver,wall_time,submit_time,folder,resourcepath,targetpath,status]
                    task_line=[job_id,task['job_name'],task['user_name'],task['charge_number'],task['solver'],task["args"],task["wall_time"],task["submit_time"],task["folder"],tfgp,tgp,"waiting"]
    ...
    def task_solver(self):
        ...
        with open ("run.bat","w") as f:
            if data[4] == "abaqus" :
                ...
                print ("call C:/SIMULIA/Commands/abaqus.bat job=" + str(data[1])+" "+str(data[5])+" " +"int",file = f )
                print ("del *.env",file = f)
                print ("",file=f)
            elif data[4] =="nastran" :
                 # print("ping -n 100 www.baidu.com",file=f)
                print ('call "C:/Program Files/Siemens/NX1872/NXNASTRAN/bin/nastran.exe " ' + str(data[1])+".bdf"+ " parallel=56 "+" "+"old=no",file = f)
    ...
    def taskfile_tf(self,file):
        ...
        tasklist.extend(self.task_list_waiting)
    ...
    def task_monitor(self):
        ...
        return task_monitor
    def task_csv(self):
        csv_root = "E:\\WeiLi\\Personal\\Python\\Test\\ServerA\\jobs\\Databas.csv"
        tasklist = []
        tasklist.extend(self.task_list_finished)
        ...
        with open (csv_root,"w+") as f :
            ...
            csvw.writerow([ "job_id", "job_name","user_name","charge_number","solver","wall_time","submit_time","folder","resourcepath","targetpath","status"])
            csvw.writerows(tasklist)
